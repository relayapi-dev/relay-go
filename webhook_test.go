...
func TestWebhookUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Webhooks.Update(
		...
		relaygo.WebhookUpdateParams{
			...
			Events:  relaygo.F([]relaygo.WebhookUpdateParamsEvent{relaygo.WebhookUpdateParamsEventPostPublished}),
			URL:     relaygo.F("https://example.com"),
		},
	)
	if err != nil {
...
func TestWebhookListLogsWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestWebhookSendTest(t *testing.T) {
	...
	_, err := client.Webhooks.SendTest(context.TODO(), relaygo.WebhookSendTestParams{
