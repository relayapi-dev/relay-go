...
```go
import (
	"github.com/relayapi-dev/relay-go" // imported as relaygo
)
```
...
```sh
go get -u 'github.com/relayapi-dev/relay-go@v0.2.0'
```
...
```go
params := FooParams{
	Name: relaygo.F("hello"),

	// Explicitly send `"description": null`
	Description: relaygo.Null[string](),

	Point: relaygo.F(relaygo.Point{
		X: relaygo.Int(0),
		Y: relaygo.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: relaygo.Raw[int64](0.01), // sends a float
	}),
}
```
...
See the [full list of request options](https://pkg.go.dev/github.com/relayapi-dev/relay-go/option).
...
`*relaygo.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `relaygo.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   relaygo.F("id_xxxx"),
    Data: relaygo.F(FooNewParamsData{
        FirstName: relaygo.F("John"),
    }),
}
