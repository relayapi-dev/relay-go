...
func TestInboxReviewReplyNew(t *testing.T) {
	...
	_, err := client.Inbox.Reviews.Reply.New(
		context.TODO(),
		"review_id",
...
func TestInboxReviewReplyDelete(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Inbox.Reviews.Reply.Delete(context.TODO(), "review_id")
	if err != nil {
		var apierr *relaygo.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
