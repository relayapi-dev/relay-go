...
func TestAccountGmbLocationGet(t *testing.T) {
	...
	_, err := client.Accounts.GmbLocations.Get(context.TODO(), "id")
	if err != nil {
...
func TestAccountGmbLocationSetDefault(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Accounts.GmbLocations.SetDefault(
		context.TODO(),
		"id",
