...
func TestWhatsappGroupNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Whatsapp.Groups.New(context.TODO(), relaygo.WhatsappGroupNewParams{
...
func TestWhatsappGroupList(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Whatsapp.Groups.List(context.TODO(), relaygo.WhatsappGroupListParams{
		AccountID: relaygo.F("account_id"),
	})
	if err != nil {
		var apierr *relaygo.Error
...
func TestWhatsappGroupDelete(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
